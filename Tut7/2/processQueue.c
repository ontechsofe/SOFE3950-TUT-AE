...
Process delete_name(ProcessQueue _processQueue, char _name[256]) {
    queue_t *queue = (queue_t *)_processQueue;
    queue_element_t *current = queue->head;
    queue_element_t* previous = NULL;

    if (queue->head == NULL) {
        return NULL;
    }

    while (strcmp(current->curr->name, _name) != 0) {
...
Process delete_pid(ProcessQueue _processQueue, int pid) {
    queue_t *queue = (queue_t *)_processQueue;
    queue_element_t *current = queue->head;
    queue_element_t* previous = NULL;
